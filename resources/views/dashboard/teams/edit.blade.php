        ...
        <div class="container-fluid">
            <div class="row clearfix">
                <div class="col-md-12">
                    <div class="card">
                        <div class="header">
                            <h2><strong>تعديل</strong> الفريق</h2>
                        </div>

                        <div class="body">
                            <form action="{{route('dashboard.teams.update', $team)}}" method="POST"
                                    ...
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <input type="text" name="name" class="form-control"
                                                   placeholder="إسم الفريق" value="{{ $team->name }}">
                                            @error('name')
                                    ...
                                    <div class="col-sm-6">
                                        <select class="form-control z-index show-tick nominate_beneficiary"
                                            ...
                                            @foreach($jobSeekers as $jobSeeker)
                                                <option {{ $team->leader_id === $jobSeeker->id ? 'selected' : '' }} value="{{ $jobSeeker->id }}">
                                                    {{ $jobSeeker->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            <textarea name="bio" rows="4" class="form-control no-resize"
                                                      placeholder="نبذة...">{{ $team->bio }}</textarea>
                                            @error('bio')
                                            ...
                                            @foreach($team->members as $index=>$member)
                                                <tr>
                                                    <td>
                                                        <select class="form-control z-index show-tick nominate_beneficiary"
                                                                data-live-search="true"
                                                                name="members[{{$index}}][id]" required>
                                                            <option selected disabled>- باحث عن عمل -</option>
                                                            @foreach($jobSeekers as $jobSeeker)
                                                                <option {{ $member->id === $jobSeeker->id ? 'selected' : '' }} value="{{ $jobSeeker->id }}">
                                                                    {{ $jobSeeker->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
