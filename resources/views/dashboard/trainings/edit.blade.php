        ...
        <div class="container-fluid">
            <div class="row clearfix">
                <div class="col-md-12">
                    <div class="card">
                        <div class="header">
                            <h2><strong>تعديل</strong> التدريب</h2>
                        </div>

                        <div class="body">
                            <form action="{{route('dashboard.trainings.update', $training)}}" method="POST">
                                @csrf
                                @method('PUT')
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            <input type="text" name="title" class="form-control"
                                                   placeholder="العنوان" value="{{ $training->title }}">
                                            @error('title')
                                    ...
                                    <div class="col-sm-4">
                                        <select class="form-control z-index show-tick nominate_beneficiary"
                                            ...
                                            @foreach($employers as $employer)
                                                <option {{ $training->employer_id === $employer->id ? 'selected' : '' }} value="{{ $employer->id }}">
                                                    {{ $employer->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-4">
                                        <select class="form-control z-index show-tick nominate_beneficiary"
                                            ...
                                            @foreach($categories as $category)
                                                <option {{ $training->category_id === $category->id ? 'selected' : '' }} value="{{ $category->id }}">
                                                    {{ $category->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-4">
                                        <select class="form-control z-index show-tick nominate_beneficiary"
                                            ...
                                            @foreach($regions as $region)
                                                <option {{ $training->region_id === $region->id ? 'selected' : '' }} value="{{ $region->id }}">
                                                    {{ $region->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            <textarea name="description" rows="7" class="form-control no-resize"
                                                      placeholder="وصف التدريب...">{{ $training->description }}</textarea>
                                            @error('description')
                                    ...
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            <textarea name="requirement" rows="7" class="form-control no-resize"
                                                      placeholder="المتطلبات...">{{ $training->requirement }}</textarea>
                                            @error('requirement')
