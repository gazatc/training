    ...
    <div class="text-center bg-white rounded-lg md:w-1/2 m-auto ">
        <header>
            <h2 class="rounded-t-lg text-gray-800 uppercase text-center tracking-wide
                text-xl font-semibold mb-2 py-1 bg-gray-100">تعديل التدريب </h2>
        </header>
        ...
        @endif
        <form action="{{route('training.update',$training)}}" method="post" class="px-4 py-2" enctype="multipart/form-data">
            @csrf
                ...
                <div class="py-1">
                    ...
                    <div class="pt-3">
                        <input type="text" required name="title" value="{{$training->title}}"
                    ...
                    <div class="pt-3">
                        <select required name="region"
                            ...
                            @foreach($regions as $region)
                                <option value="{{$region->id}}" @if($region->id == $training->region_id) selected @endif>{{$region->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('region')
                    ...
                    <div class="pt-3">
                        <select required name="category"
                            ...
                            @foreach($categories as $category)
                                <option value="{{$category->id}}" @if($category->id == $training->category_id) selected @endif>{{$category->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('category')
                    ...
                    <div class="pt-3">
                            <textarea  name="description" id="" cols="30" required rows="5"
                                       class="border border-gray-300 w-full text-sm rounded-sm bg-gray-100 px-3 py-1.5
                                        focus:outline-none focus:border-blue-900">{{$training->description}}</textarea>
                    </div>
                    @error('description')
                ...
                <div class="py-2">
                    ...
                    <div class="pt-3">
                            <textarea  name="requirement" id="" cols="30" required rows="5"
                                       class="border border-gray-300 w-full text-sm rounded-sm bg-gray-100 px-3 py-1.5
                                       focus:outline-none focus:border-blue-900">{{$training->requirement}}</textarea>
                    </div>
                    @error('requirement')
                ...
                <div class="py-1">
                    ...
                    <div class="pt-3">
                        <input type="date" name="last_date" required   value="{{$training->last_date}}"
                               class="border border-gray-300 w-full text-sm rounded-sm bg-gray-100 px-3 py-1.5 focus:outline-none focus:border-blue-900">
                    </div>
                    @error('last_date')
